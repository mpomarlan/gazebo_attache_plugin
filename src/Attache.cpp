...
namespace gazebo {
  ...
  void Attache::Load(physics::WorldPtr wpWorld, sdf::ElementPtr epPtr) {
    ...
    std::cout << "Attache plugin loaded" << std::endl;
  }
  ...
  bool Attache::serviceAttach(attache_msgs::Attachment::Request &req, attache_msgs::Attachment::Response &res) {
	...
	if(mpLink2) {
	  ...
	  if(lpLink2) {
	    ...
	    std::cout << "[Attache] Attach link '" << lpLink1->GetName() << "' to link '" << lpLink2->GetName() << "'" << std::endl;
	    ...
	    std::cerr << "[Attache] No link '" << req.model2 << "." << req.link2 << "'" << std::endl;
	  }
	} else {
	  std::cerr << "[Attache] No model '" << req.model2 << "'" << std::endl;
	}
      } else {
	std::cerr << "[Attache] No link '" << req.model1 << "." << req.link1 << "'" << std::endl;
      }
    } else {
      std::cerr << "[Attache] No model '" << req.model1 << "'" << std::endl;
    }
  ...
  bool Attache::serviceDetach(attache_msgs::Attachment::Request &req, attache_msgs::Attachment::Response &res) {
    ...
    if(this->deleteJointIfPresent(strLink1, strLink2) || this->deleteJointIfPresent(strLink2, strLink1)) {
      ...
      std::cerr << "[Attache] No connection to detach between '" << strLink1 << "' and '" << strLink2 << "'" << std::endl;
